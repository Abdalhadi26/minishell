...
/*   Updated: 2026/06/07 16:19:46 by ahhammad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_next_tokenaa(t_lexer f_token, t_lexer *s_token,t_redirections *here)
{
	if (!s_token)
		return (1);
	else if (!s_token->qouted && check_input_redaa(f_token, 0))
	{
		if (s_token_check_o_paa(*s_token) || s_token_check_i_paa(*s_token))
			return (1);
		else if (check_input_redaa(f_token, 0) == 2)
		{
			here->next = init_red(3, &f_token);
			if (!here->next)
				return (1);
