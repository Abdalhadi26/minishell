...
/*   Updated: 2026/06/10 10:38:41 by ahhammad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* execute */
void						execute_single(t_command_list *cmds,
								t_single_command command, t_shell *shell);
void						execute_pipeline(t_command_list *command,
								t_shell *shell);
void						close_all_pipes(int **pipes, int num_pipes);
void						free_pipes(int **arr, int n);
int							create_pipes(t_command_list command, int ***pipes);
void						child_cleanup_exit(t_command_list *command,
								t_shell *shell, int **pipes, int code);
...
int							collect_heredocs(t_command_list *command,
								t_shell *shell);
int							expand_heredoc(char *line, int *pipe_fd, int qouted,
								t_shell *shell);
int							handle_heredoc_eof(t_redirections *redir,
								int *pipe_fd, int saved_stdin);

/* builtins */
int							is_builtin(char *cmd);
int							execute_builtin(t_command_list *cmds,
								t_single_command command, t_shell *shell);
int							builtin_cd(t_single_command cmd, t_shell *shell);
int							builtin_echo(t_single_command cmd);
int							builtin_env(t_shell shell);
int							builtin_exit(t_command_list *cmds,
...
void						free_cmds_shell(t_command_list *cmds,
								t_shell *shell);
void						close_other_heredocs(t_command_list *command,
								int i);
